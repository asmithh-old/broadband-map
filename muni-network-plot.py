...
import re
#you don't actually need scipy unless you want to do regressions on the points that are plotted, so if it's problematic comment out the line importing scipy.
...
############LEAVE THIS BLOCK UNCOMMENTED TO SEE (POP DENSITY, MUNI PEN) AND (POP DENSITY, NOT MUNI PEN) plots.
###############################################################################################################
...
#to get best-fits, stats.mstats.thiel(<y-var>, <x-var>). best to print it to terminal, I think--the output is messy.
#first output is slope; second output is intercept.
###############################################################################################################
###############################################################################################################
    
################LEAVE THIS BLOCK UNCOMMENTED TO SEE MUNI:NOT MUNI RATIOS AND BEST FIT LINE#####################
###############################################################################################################
for i in range(len(muniPens)):
